...
void destroy_string_function(void* s){
    free((char*)s);
}
void* copy_string_function(const void* s){
    char* cpy;
    if(!s){
        return NULL;
    }
    cpy=(char*)calloc(strlen((char*)s)+1, sizeof(char));
    if(!cpy){
        return NULL;
    }
    strcpy(cpy, (char*)s);
...
int print_string_function(FILE *f, const void*s){
    return fprintf(f, "[%s] ", (char*)s);
}

int cmp_string_function(const void* s1, const void* s2){
    return strcmp(s1, s2);
}
